  ...
  const options = {
    ...
    scales: {
      y: {
        ...
        min:
          Math.min(...chartData?.map((el) => Math.round(+el.sum * 24230))) - 5 <
          0
            ? 0
            : Math.min(...chartData?.map((el) => Math.round(+el.sum * 24230))) -
              5,
        max:
          Math.max(...chartData?.map((el) => Math.round(+el.sum * 24230))) + 5,
        border: { dash: [20, 0] },
      },
      x: {
        ticks: { color: "black" },
